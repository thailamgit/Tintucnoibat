...
class Breadcrumb_Trail {
    ...
    protected function set_labels() {
        ...
        $defaults = array(
            'browse'              => esc_html__( 'Browse:',                               'newsis' ),
            'aria_label'          => esc_attr_x( 'Breadcrumbs', 'breadcrumbs aria label', 'newsis' ),
            'home'                => esc_html__( 'Home',                                  'newsis' ),
            'error_404'           => esc_html__( '404 Not Found',                         'newsis' ),
            'archives'            => esc_html__( 'Archives',                              'newsis' ),
            // Translators: %s is the search query.
            'search'              => esc_html__( 'Search results for: %s',                'newsis' ),
            // Translators: %s is the page number.
            'paged'               => esc_html__( 'Page %s',                               'newsis' ),
            // Translators: %s is the page number.
            'paged_comments'      => esc_html__( 'Comment Page %s',                       'newsis' ),
            // Translators: Minute archive title. %s is the minute time format.
            'archive_minute'      => esc_html__( 'Minute %s',                             'newsis' ),
            // Translators: Weekly archive title. %s is the week date format.
            'archive_week'        => esc_html__( 'Week %s',                               'newsis' ),
    ...
    protected function add_minute_hour_archive_items() {
        ...
        if ( true === $this->args['show_title'] )
            $this->items[] = sprintf( $this->labels['archive_minute_hour'], get_the_time( esc_html_x( 'g:i a', 'minute and hour archives time format', 'newsis' ) ) );
    }
    ...
    protected function add_minute_archive_items() {
        ...
        // Add the minute item.
        if ( true === $this->args['show_title'] )
            $this->items[] = sprintf( $this->labels['archive_minute'], get_the_time( esc_html_x( 'i', 'minute archives time format', 'newsis' ) ) );
    }
    ...
    protected function add_hour_archive_items() {
        ...
        // Add the hour item.
        if ( true === $this->args['show_title'] )
            $this->items[] = sprintf( $this->labels['archive_hour'], get_the_time( esc_html_x( 'g a', 'hour archives time format', 'newsis' ) ) );
    }
    ...
    protected function add_day_archive_items() {
        ...
        $year  = sprintf( $this->labels['archive_year'],  get_the_time( esc_html_x( 'Y', 'yearly archives date format',  'newsis' ) ) );
        $month = sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'newsis' ) ) );
        $day   = sprintf( $this->labels['archive_day'],   get_the_time( esc_html_x( 'j', 'daily archives date format',   'newsis' ) ) );
    ...
    protected function add_week_archive_items() {
        ...
        $year = sprintf( $this->labels['archive_year'],  get_the_time( esc_html_x( 'Y', 'yearly archives date format', 'newsis' ) ) );
        $week = sprintf( $this->labels['archive_week'],  get_the_time( esc_html_x( 'W', 'weekly archives date format', 'newsis' ) ) );
    ...
    protected function map_rewrite_tags( $post_id, $path ) {
        $site_date_to_show = NI\newsis_get_customizer_option( 'site_date_to_show' );
        $post = get_post( $post_id );
            ...
            foreach ( $matches as $match ) {
                ...
                if ( '%year%' == $tag ) {
                    if( $site_date_to_show == 'modified' ) {
                        $this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_year_link( get_the_modified_time( 'Y', $post_id ) ) ), sprintf( $this->labels['archive_year'], get_the_modified_time( esc_html_x( 'Y', 'yearly archives date format',  'newsis' ), $post_id ) ) );
                    } else {
                        $this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_year_link( get_the_time( 'Y', $post_id ) ) ), sprintf( $this->labels['archive_year'], get_the_time( esc_html_x( 'Y', 'yearly archives date format',  'newsis' ), $post_id ) ) );
                    }
                
                // If using the %monthnum% tag, add a link to the monthly archive.
                } elseif ( '%monthnum%' == $tag ) {
                    if( $site_date_to_show == 'modified' ) {
                        $this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_month_link( get_the_modified_time( 'Y', $post_id ), get_the_modified_time( 'm', $post_id ) ) ), sprintf( $this->labels['archive_month'], get_the_modified_time( esc_html_x( 'F', 'monthly archives date format', 'newsis' ), $post_id ) ) );
                    } else {
                        $this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_month_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ) ) ), sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'newsis' ), $post_id ) ) );
                    }
                // If using the %day% tag, add a link to the daily archive.
                } elseif ( '%day%' == $tag ) {
                    if( $site_date_to_show == 'modified' ) {
                        $this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_day_link( get_the_modified_time( 'Y', $post_id ), get_the_modified_time( 'm', $post_id ), get_the_modified_time( 'd', $post_id ) ) ), sprintf( $this->labels['archive_day'], get_the_modified_time( esc_html_x( 'j', 'daily archives date format', 'newsis' ), $post_id ) ) );
                    } else {
                        $this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_day_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ), get_the_time( 'd', $post_id ) ) ), sprintf( $this->labels['archive_day'], get_the_time( esc_html_x( 'j', 'daily archives date format', 'newsis' ), $post_id ) ) );
                    }
